...
We can then run `npm run dev` to start our application.

### 4.3. Ignoring Files with Webpack

By importing libraries into start files, all the libraries will be loaded into the file generated by webpack. That's what we want for client-side file, but it's a different story for server-side `bundle.js` file. Since we can require node modules at runtime with node, we don't need everything to pass webpack when our server first starts up.

How we can keep webpack from importing our libraries into our server bundle? We need to use a little module called `webpack-node-externals`.
...
```
If we restart our application now, the size of server bundle went from 712 KB to 4.4 KB:

![10](./images/01/01-10.png "10") 
![11](./images/01/01-11.png "11") 

### 4.4. Renderer Helper

`/server/src/index.js` will be messy if we have lots of code for rendering. We need to file to simply render our React app and return with a string:
